...
- **Raw views**
  - `auto* GetPtr() noexcept` — pointer to the beginning of the field inside the buffer.
...
> When returning a scalar through a variant, the implementation may construct a temporary `T` value. For zero‑copy needs prefer `GetPtr()` / span‑based views where applicable.
...
Payload data;
auto* p = data.GetPtr();                 // raw pointer into the shared buffer
...
- Favor `GetPtr()`/span for bulk payload handling to stay zero‑copy.
